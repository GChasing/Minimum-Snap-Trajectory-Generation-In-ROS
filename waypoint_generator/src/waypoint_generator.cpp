...
{
    std::cout << "enter publish_waypoints()" << std::endl;
...
{
    //std::cout << "enter publish_waypoints_vis()" << std::endl;
...

    {
        ...
        //std::cout << "poseArray.poses.push_back(init_pose) = " << poseArray.poses[0] << std::endl;
    }
    for (auto it = waypoints.poses.begin(); it != waypoints.poses.end(); ++it)
    {
        ...
        //std::cout << "poseArray.poses[i] = " << *it << std::endl;
    }
    //测试输出
    // for(auto it=poseArray.poses.begin(); it!=poseArray.poses.end(); ++it)
    // {
...
{
    std::cout << "enter odom_callback()" << std::endl;
    is_odom_ready = true;
    odom = *msg;
    ...
    if (waypointSegments.size())
    {
        ...
        if (odom.header.stamp >= expected_time)
        {
            ...
            std::cout << "end odom_callback()" << std::endl;
        }
    }
}
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    std::cout << "enter goal_callback()" << std::endl;
    ...
    else    // manual several waypoints
    {   
        if (msg->pose.position.z > 0)
        {
            //std::cout << "enter  if (msg->pose.position.z > 0)" << std::endl;
        ...
        {
            //std::cout << "enter  if (msg->pose.position.z > -1.0)" << std::endl;
        ...
        {
            //std::cout << "if -1.0 > height, end of input" << std::endl;
...
{
    std::cout << "enter traj_start_trigger_callback()" << std::endl;
    if (!is_odom_ready)
    {
        ROS_ERROR("[waypoint_generator] No odom!");
        return;
